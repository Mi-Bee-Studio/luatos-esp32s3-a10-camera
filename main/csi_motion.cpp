...
/* luatos 桥接（2026-09-07）：CSI 事件经板内 ws_broadcast_text 推 SPA（契约
 * v1.4 §6）。该实现内部已 fd+httpd_ws_send_frame_async（跨任务安全），
 * 无客户端时近零开销——无 PSRAM 板堆预算可承受。回调上下文=runtime loop
 * 任务（即本模块的 pump 任务），满足 SDK 有界回调契约。 */
#include <ctime>
extern "C" esp_err_t ws_broadcast_text(const char *msg, size_t len);
...
public:
    void on_motion_state_changed(const espectre::RuntimeSnapshot &s) override {
        ...
        /* 契约 §6：motion_* 的 score 为家族 0-100 刻度，source:csi 供 UI 区分 */
        char msg[128];
        int n = snprintf(msg, sizeof(msg),
                         "{\"type\":\"%s\",\"timestamp\":%lld,"
                         "\"data\":{\"score\":%.0f,\"source\":\"csi\"}}",
                         s.motion_state == espectre::MotionState::MOTION
                             ? "motion_started" : "motion_cleared",
                         (long long)time(NULL), s.movement_metric * 100.0f);
        if (n > 0 && n < (int)sizeof(msg)) {
            ws_broadcast_text(msg, (size_t)n);
        }
    }
    ...
    void on_periodic_update(const espectre::RuntimeSnapshot &s,
         ...
         * profile/locked/calibrating/flip_rate）。板内 ws_broadcast_text
         * 组完整信封（跨任务安全，无客户端近零开销）。 */
        char msg[192];
        int n = snprintf(msg, sizeof(msg),
                         "{\"type\":\"csi_status\",\"timestamp\":%lld,"
                         "\"data\":{\"state\":\"%s\",\"score\":%.2f,\"thr\":%.2f,"
                         "\"profile\":%u,\"locked\":%s,\"calibrating\":%s,\"flip_rate\":%u}}",
                         (long long)time(NULL), st, s.movement_metric, s.threshold,
